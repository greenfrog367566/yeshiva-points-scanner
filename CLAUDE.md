...
- **Firebase_Rebuild_Scope.md** — the settled scope for the Firebase/Firestore rebuild: real accounts, Firestore replacing localStorage-as-database, three tiers (rebbi/admin/superadmin), the incremental-write data model (not one JSON blob per class), the converter tool, what retires (file:// offline copy, Sheets-as-database, Apps Script), and the 8-step build order. **Built and deployed as of 2026-08-18** — all 8 build-order steps merged (PRs #290, #292, #300, #303, #309) and `firebase deploy` is live against `menchmark-backend`. Read before touching anything auth/sync/data-model shaped. **But "built and deployed" does not mean a rebbi's class data is actually in Firestore** — verified 2026-08-20: there was no live write-sync path for tier-1's day-to-day scans/points/gradebook, which live only in `localStorage` exactly as before the rebuild; only a one-time converter snapshot ever reaches it. The fix is designed (`docs/Firebase_TierOne_WriteSync_Design_Proposal.md`, decided 2026-08-21) but **not yet built** — see `docs/NOW.md` → "Merged, not done." Step 8 (migrating the real beta cohort) is the only build-order step left, and it should not proceed until this gap is closed.
...
- **Tracked activities never award points by default** — an activity's `pts` is 0 and locked in the editor unless explicitly opted into (see below). **They are no longer excluded from arming surfaces.** Since 2d every tracked activity is armable everywhere (Record-tab buttons, floating scan panel, seating-chart dropdown, `ACT:` codes, printed Activity Menu), because `award()` routes a tracked scan to `recordTrackedScan()` rather than letting it fall through to the points path. The old Homework-Checked-only asymmetry in the seating-chart dropdown is gone — it existed solely because that one activity had a reroute and the others didn't. `reroutedActivity(a)` is now just `a.tracked===true`.
  - **Amended 2026-08-20 — Homework Checked is the one opt-in exception, via `syncHwPoints()`.** A rebbi can give it a real `a.pts` in the Activities editor; points then track TODAY'S CHECKED STATE, not the scan — they land the moment a day's record first reads `checked===true` and reverse the moment it doesn't, regardless of whether that flip came from a scan, the Homework tab's own toggle, or a Gradebook correction to a past day. This is deliberate: it is NOT event-sourced (award N points per scan), because `markHwCheckedToday()`'s idempotent re-scan of an already-checked folder would otherwise double-award, and a correction un-checking a day would otherwise strand the points it gave. State-sync is implemented once, inside the three places a day's record can ever change — `setHw()`, `markHwCheckedToday()`, `clearHomework()` — so every existing write path (scan, tab toggle, Gradebook correction, "Undo last scan," a row's own ↩ in History) reconciles points automatically with no special-casing per path. A past-day correction is deliberately NOT contest-tagged, same reasoning as `applyScore()`'s `opts.noContest`. Every OTHER tracked activity (Attendance, Bathroom Pass, Freeze, a rebbi's own Tracker items) still cannot carry points — each has its own correction/cycle semantics (attendance is a genuine Present→Late→Absent cycle with no "already marked" guard; a naive "add points on write" rule would double-award on every re-cycle) that a state-sync or event-sourced model hasn't been designed against yet. Extending this to another tracked activity is a fresh PROPOSE FIRST, not an assumed next step.
