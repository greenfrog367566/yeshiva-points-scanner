...
- **Menchmark_Phased_Build_Plan.md** — the build order (Phase 0→8). Don't build a phase whose dependencies aren't in yet. **That doc is the authority on phase status** — it carries a dated status stamp per phase; read those rather than relying on a summary here, which is how this paragraph went two phases stale. As of 2026-08-05: **Phase 0 and 1 DONE**; **Phase 2 — 2a (#107) and 2b (#115) SHIPPED, 2c PARTIAL** (only `data.attendance` converted, PR #138; `data.hw` resets and `data.trackerLog` / `data.passes` are TBD, remainder in #122), **2d SHIPPED (#123)** in two parts — the scan mechanic (#208) and the tile badges. **Contest is un-hidden again** (#210). **The Gradebook tab is STILL hidden (#185), and 2d landing did not change that**: scans now write `data.trackedData`, but corrections made on the four old tabs still do not reach it, so the grid would contradict the tab a rebbi just fixed. Closing that mirror gap at the legacy setters is what un-hides it. 2d has, however, cleared Phase 5 and the Firebase rebuild's step 1 — it settled the count value shape those were waiting on. **Phase 6a is data-only and orphaned (#187)**; 6b was never built. Phase 3 is 1-of-5, Phase 4 is 0-of-3.
...
- **Tracked scans DUAL-WRITE, and `TRACKED_LEGACY` is 2c's deletion list.** 2c is partial (#122): the four old stores still drive four visible tabs, so a tracked scan writes the legacy store *and* mirrors into `data.trackedData`. The legacy hook owns the value and the mirror records what it returns — never re-derived, or the two would drift. When #122 retires a tab, delete its row from `TRACKED_LEGACY`. **Corrections made on the old tabs do not reach the mirror yet** — that gap is what still blocks un-hiding the Gradebook (#185), not 2d itself.
