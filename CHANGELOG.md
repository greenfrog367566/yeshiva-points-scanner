...
- **"Prize Received" tracked activity**, seeded automatically the same way "Homework Checked" is — a built-in `tracked:true`, 0-point activity that immediately works everywhere a normal tracked activity does: selectable on the Tracker tab, included on Personal scoring pages' printed QR grid, and shown in that student's history — no dedicated tab needed.
