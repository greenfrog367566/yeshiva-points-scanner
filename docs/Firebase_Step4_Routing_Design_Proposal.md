...
With `classId` established as a first-class routing concern — a resolvable, guardable, history-trailed unit rather than an implicit ambient value — step 5 (superadmin tools) can build cross-class/cross-school views on top of `router.listSwitchTargets()`-shaped listing without inventing its own class-resolution logic, and step 6 (admin's gradebook view) can build its class picker directly against the three routing-only primitives (`switchClass`, `currentClassId`, `listSwitchTargets`) documented above rather than reaching into module-level state. Both steps inherit a working, back-button-correct navigation substrate instead of needing to design one themselves.
