...
**Second, separate console gap found the same session:** even past the OAuth block, automatic Drive backup fails with `403 Google Drive API has not been used in project 566788715634 before or it is disabled` — the Drive API itself was never enabled on the `menchmark-backend` GCP project (Firebase enabling Auth/Firestore doesn't enable it). Fix is a one-click **Enable** at `console.developers.google.com/apis/api/drive.googleapis.com/overview?project=566788715634`, then retry (Google's own message warns propagation can take a few minutes). Same "Ben's console access only" shape as the OAuth blocker above — both need clearing before step 8's Drive backup can work for anyone.
