...
**Steps 2, 3, 3b, and the sign-in UI are MERGED to `main`** (PR #290, `feat/firebase-step2-auth-rules`): `firestore.rules`' full tier-gating matrix; three Cloud Functions (`redeemCode`, `provisionRebbi`, `viewAs`); `provisionRebbi`'s `admin-invite`/`roster`/`backup` mode dispatch with a chunked-write-plus-verification-receipt engine; the Firebase SDK vendored for real; a superadmin converter UI (`tools/admin-convert.html`); and the full SDK-free-until-tier-1 sign-in flow in `app.html` (OAuth redirect, code entry, same-device adoption offer, self-serve roster).

**Step 4** (routing/back-button + tier guard), built and in review — PR #292, draft, CI green: all 5 PRs of its own incremental sequence, including the tier-aware route guard. Real hash-based routing, working Back/Forward, deep links, a fail-closed `view-routeError` screen, every user-clickable tab-jump routed through `router.go()`.

**Steps 5 + 6** (superadmin tools + admin's Class Book), built and in review — PR #300, `feat/step5-superadmin-tools`, draft, CI green: a new standalone `admin.html` (activity overview, view-as, email export), plus a Firestore-read pipeline (`fetchClassFromFirestore()`) that neither step originally had — built once, shared by both, since step 6's Admin Class Book needed it and it's what makes step 5's view-as land on a real screen instead of a banner over nothing.

**Step 7** (the fragile-storage warning), built and in review on this branch — `feat/step7-fragile-storage-warning`: a true retarget of the existing backup-staleness nudge (red/urgent variant, fires immediately on unpersisted storage, ignoring the 7-day floor). **Building it surfaced that its tier-2 branch assumed Google Drive was already wired into `app.html` — it wasn't**, only a standalone spike (`spikes/drive-oauth-spike.html`) had ever proven the mechanism. This PR ports it for real: a new Settings → Drive backup card, OAuth token in memory only (never persisted), silent re-auth via a hidden iframe with `login_hint`, a third hash-fragment consumer disambiguated by a `state` prefix from both step 4's router and the sign-in flow's own OAuth capture. **Three real bugs found and fixed in browser testing** (a silent `ReferenceError` from a wrong-scope variable; an implicit global from a non-strict-mode script silently writing to a *different* variable of the same name; a synchronous throw escaping a promise `.catch()`) — see `CHANGELOG.md` for the full account; none were catchable by any static check, all three surfaced only by actually clicking the button and reading the console. **One deliberate deviation from the design doc, flagged rather than silently followed:** tier-1's "nothing is at risk" suppression needs a live Firestore write-sync path from a rebbi's own scans that doesn't exist anywhere yet — out of scope for this pass, so every account sees the same warning today, tier-1 included. **Still owed, unchanged from the design:** the three-part managed-Chromebook verification session (Ben's own hardware, can't be simulated) — step 7 must not ship to beta rebbeim until it runs and passes.

**Design phase is now fully spent** — steps 0 through 7 all have code behind them (some merged, some in the two remaining open PRs). What's left in the whole rebuild is step 8: migrating the real beta cohort, which needs both open PRs (#292, #300) merged and deployed first.

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`) — PR #292 (step 4) and PR #300 (steps 5+6) are both still open/draft; this step-7 branch's own PR will be a third.
