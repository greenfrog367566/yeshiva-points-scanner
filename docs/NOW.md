...
- **Step 4 (routing/back-button), this PR, is now fully done — all 5 PRs of its own incremental sequence, including the tier guard.** PR1–3 and PR5 (module, back-button wiring, deep-link cold start, opportunistic audit) shipped first; **PR4 (the tier-aware route guard) shipped without waiting on step 2's sign-in UI** — it doesn't need a real signed-in session to be *correct*, only to ever exercise its tier-1 branch, so it ships live with a real fail-closed check (an unknown/bad classId already shows the new `view-routeError` screen instead of silently rendering) rather than staying dormant code nothing calls. `CLASS_INDEX`/`currentAuthUid()` are forward-compatible with step 2's shape once that PR merges — nothing here needs to change when it does. See `CHANGELOG.md` for the full breakdown.
- **Step 2 (auth + Cloud Functions + `firestore.rules`) + step 3 (the converter tool) + the sign-in UI are built on a separate PR** (`feat/firebase-step2-auth-rules`) — `firestore.rules`'s full tier-gating matrix, `redeemCode`/`provisionRebbi`/`viewAs` Cloud Functions, the superadmin converter (`tools/admin-convert.html`), and the real Google sign-in screens in `app.html` (OAuth redirect, code entry, same-device adoption offer, self-serve roster). CI green on both PRs as of this session.

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`) — both PRs are still draft. Once step 2's PR merges, this branch's `CLASS_INDEX`/`currentAuthUid()` extend naturally to real multi-class tier-1 data — that's what makes PR4 correct-but-inert today rather than needing a rebuild later.
