...
- **Step 5 (superadmin tools) + step 6 (admin Class Book / read pipeline)** — `feat/step5-superadmin-tools`, PR #300, draft, CI green: `admin.html` (activity overview, view-as flow, email export), `activitySummary`/`viewAsLog`/`auditLog` collections and their Cloud Functions, plus — discovered as a gap mid-step-6 and built the same PR — the first real Firestore-read pipeline (`fetchClassFromFirestore()`), since nothing before this could read a class's live data back out. Reuses the existing Gradebook table-building code (`gbBuildTable()` etc.) rather than forking it.
