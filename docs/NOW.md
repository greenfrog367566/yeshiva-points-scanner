...
**What's left in the whole rebuild is step 8** (migrating the real beta cohort) — not started. It waits on two things neither of which is more code: **the managed-Chromebook verification sessions** step 0c and step 7 each still owe (can't be simulated, needs Ben's own hardware), and **a real Firebase deploy** — nothing has reached the live project yet.

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`) — this is now the actual next gate, not another PR.
