...
**Implementation started 2026-08-16, once Ben created the real `menchmark-backend` Firebase/GCP project.** Steps 2 and 3 are built and in review (PR #290, draft, CI green): `firestore.rules`' full tier-gating matrix, three Cloud Functions (`redeemCode`/`provisionRebbi`/`viewAs`), `provisionRebbi`'s mode dispatch (admin-invite/roster/backup, admin-driven or self-serve) with a chunked-batch-write-plus-verification-receipt converter engine, the Firebase SDK vendored for real (`vendor/firebase/`), and a superadmin converter UI (`tools/admin-convert.html`). Step 4 (routing/back-button) is also built (PR #292, draft, CI green) — all 5 PRs of its own incremental sequence, browser-verified: real hash-based routing, working Back/Forward, deep links, and every user-clickable tab-jump routed through `router.go()`.

**The critical path now is the sign-in UI — `docs/Firebase_SignIn_UI_Design_Proposal.md`, PROPOSAL, not yet approved.** Discovered mid-build: step 2's doc specifies the sign-in *flows* (screens, reads, writes) and step 4's doc assumes sign-in UI exists, but nothing actually designed the screens — where they live in `app.html`, the SDK-free-until-tier-1 loading mechanism, or how this coexists with the tier-2/offline "never a wall" promise. Three things are already built and stalled on this: step 3's self-serve restore mount (backend done, no screen to call it from), step 4's PR4 (tier guard — can't be tested without a real signed-in session, so it was deliberately left unbuilt), and step 3b (same-device adoption offer needs a UI to surface in). **Needs Ben's review before any of it gets built** — same gate every other step's doc went through.
