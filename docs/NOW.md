...
**Implementation started 2026-08-16, once Ben created the real `menchmark-backend` Firebase/GCP project. Design phase is now fully spent — steps 0–7 are all built, spread across four PRs.**
...
- **Step 7 (fragile-storage warning)** — `feat/step7-fragile-storage-warning`, PR #303, draft, CI green: the storage-persistence warning per design, plus — discovered as a gap while building it — a full, real Google Drive backup integration ported from `spikes/drive-oauth-spike.html` into `app.html` itself (that spike was the only place Drive OAuth had ever been proven). **Three real bugs found and fixed in browser testing** (a silent `ReferenceError` from a wrong-scope variable; an implicit global from a non-strict-mode script silently writing to a *different* variable of the same name; a synchronous throw escaping a promise `.catch()`) — see `CHANGELOG.md` for the full account; none were catchable by any static check. Tier-1's suppression branch is flagged as deliberately not built: it depends on a live Firestore write-sync path from a rebbi's own scans that doesn't exist anywhere yet, so every account currently sees the same warning. **Needs a managed-Chromebook run before it ships** — see the PR description.
- **Step 3b (the existing-cohort upload path)** — `feat/step3b-existing-cohort-upload`, draft, local checks green: closes the three pieces `Firebase_Step3b_Existing_Cohort_Upload_Design_Proposal.md`'s own checklist still listed as unbuilt (same-device adoption itself shipped earlier in PR #290) — the two-device conflict screen (`already-exists` now surfaces as a real `err.code`, not just a message string), a self-serve backup-upload restore mount in Settings, and Drive-list-and-pick restore reusing step 7's Drive engine. **This is the gate itself** — per the design doc, no beta rebbi is asked to sign in until it ships, so it directly unblocks step 8. Browser-verified end to end via a temporary debug bridge + mocked `fetch()` (no live Firebase project on this machine to sign into for real); the bridge was removed before committing. See `CHANGELOG.md` for the full breakdown, including one small pre-existing bug fixed along the way.

**What's left in the whole rebuild is step 8** (migrating the real beta cohort) — not started, and it waits on PRs #303 and step 3b's PR merging and deploying first.

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`).
