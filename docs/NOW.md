...
**Step 2** (auth + Cloud Functions + `firestore.rules`), built and in review — PR #290, draft, CI green: `firestore.rules` enforces the full tier-gating matrix from `Firebase_Step2_Auth_Rules_Design_Proposal.md`; three Cloud Functions (`redeemCode`, `provisionRebbi`, `viewAs`) are the only writers of `accounts`/`codes.usedBy`; `firestore.rules.test.js` is the emulator-based verification harness (31/31 assertions passing). One doc discrepancy surfaced while writing the test suite: `Firebase_Step2_Auth_Rules_Design_Proposal.md`'s rule spec requires sign-in to read `codes` but its harness prose says "unauthenticated" — implemented per the rule spec (matches the actual sign-in flow, where Google Sign-In always precedes any code screen); flagged in the PR rather than silently edited.

**Step 3** (the converter tool), same PR #290: `provisionRebbi` now dispatches on `mode` — `admin-invite` (unchanged), `roster`, and `backup` (admin-driven or self-serve via `self:true`) — writing classes/students/trackedItems/trackedEntries/activities/log in chunked batches and storing a `classes/{id}/importReceipts/{runId}` (count parity + spot check + a `nameSplitFlags` count), proven against the real exported function on the Firestore+Auth emulators (`functions/provisionRebbi.test.js`, 8/8 passing). The Firebase SDK is vendored for real now (`vendor/firebase/*-compat.js`), and **CLAUDE.md rule 3 was amended a second time (Ben, 2026-08-16, via chat)** to let it also load in `tools/admin-convert.html` (built) and step 5's `admin.html` (not yet). `tools/converter-core.js` + `tools/admin-convert.html` give a superadmin roster-only, backup-upload-with-preview, and sequential bulk-invite UI. `app.html` gained one small addition, `window.__exportNormalized()`, so a backup is always normalized through the app's own `migrateData()`/`load2fix()` rather than a second implementation. **Also deliberately out of scope, matching the design doc's own boundary:** `prizeLedger` (raffle/auction/store), seating, and Settings aren't wired into the converter yet — covered so far: classes, students, trackedItems, trackedEntries, activities, log, scores.

**Step 4** (routing/back-button), built and in review — PR #292, draft, CI green: all 5 PRs of its own incremental sequence except the tier-guard one (see below), browser-verified — real hash-based routing, working Back/Forward, deep links, and every user-clickable tab-jump routed through `router.go()`.

**The sign-in UI gap flagged in both of the above is now closed on design AND built — `docs/Firebase_SignIn_UI_Design_Proposal.md`, APPROVED 2026-08-16 by Ben, same PR #290, CI green.** Every sign-in starts SDK-free (plain Google OAuth redirect + Firebase Auth REST API + a raw-HTTP callable-function call, same pattern the Drive spike already proved) — the vendored SDK loads only once an account turns out to be tier-1. Three non-wall entry points (a skippable `setup.html` step, a Settings → Account card, a one-time nudge banner), the full screen sequence matching step 2's already-locked reads/writes, and where step 3b's same-device-adoption offer surfaces. `provisionRebbi` gained `mode:"roster"` + `self:true` for the first-run screen. **A real bug found while implementing:** Google's OAuth redirect and step 4's router both use the URL fragment — fixed by capturing/clearing the OAuth response before anything else in the IIFE runs, verified end-to-end with a real fresh page load. **Browser-verified throughout** (entry screen, nudge, error state, signed-in state, sign-out — everything reachable without a completed live OAuth exchange, which needs Ben's deployed Cloud Functions + configured Auth provider to test for real). This unblocks: step 3's self-serve restore mount (now has a screen to call it from), step 4's tier-guard PR (now has a real signed-in session to test against — still not built, next up), and step 3b (same-device-adoption offer is wired in).

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`) — PR #290 (steps 2, 3 + sign-in UI) and PR #292 (step 4) are both still draft.
