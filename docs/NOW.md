...
**Still on Ben, and now the earliest gate of all:** a plain-language privacy note for the account record itself. Smaller than Q3's school agreement, but it comes first — it is needed before *any* self-serve signup in *either* tier, including his own school. **Drafted 2026-08-12 — `docs/Account_Privacy_Note.md`.** It is not published and not linked anywhere; what it needs is a read and **six specific answers** (contact address, deletion window, legal identity, whether view-as is logged and disclosed, breach-notification timeframe, Firestore region). Two of those are decisions rather than blanks: **whether a support view-as session is logged and the rebbi told** — cheap now, expensive to retrofit — and the breach timeframe, which must match whatever Q3's school agreement says.
...
2. **The privacy note for the account record** (item 0 above) — **drafted, `docs/Account_Privacy_Note.md`, needs a read plus six answers.** The earliest gate in the whole sign-in plan: it blocks any self-serve signup in *either* tier, including Ben's own school. It cannot ship with a placeholder left in it.
