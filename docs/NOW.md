...
**Implementation started 2026-08-16, once Ben created the real `menchmark-backend` Firebase/GCP project — running as two parallel PRs, both draft:**

- **Step 2 (auth + Cloud Functions + `firestore.rules`) + step 3 (the converter tool) + the sign-in UI are all built on one PR** (`feat/firebase-step2-auth-rules`, PR #290, CI green): `firestore.rules`'s full tier-gating matrix, `redeemCode`/`provisionRebbi`/`viewAs` Cloud Functions, `provisionRebbi`'s mode dispatch (admin-invite/roster/backup) with a chunked-batch-write-plus-verification-receipt converter engine, the superadmin converter (`tools/admin-convert.html`), and — **`docs/Firebase_SignIn_UI_Design_Proposal.md`, APPROVED 2026-08-16 by Ben, and built the same PR** — the real Google sign-in screens in `app.html` (SDK-free OAuth redirect, code entry, same-device adoption offer, self-serve roster).
- **Step 4 (routing/back-button) is this PR — now fully done, all 5 PRs of its own incremental sequence, including the tier guard.** PR1–3 and PR5 (module, back-button wiring, deep-link cold start, opportunistic audit) shipped first; **PR4 (the tier-aware route guard) shipped without waiting on step 2's sign-in UI to merge** — it doesn't need a real signed-in session to be *correct*, only to ever exercise its tier-1 branch, so it ships live with a real fail-closed check (an unknown/bad classId already shows the new `view-routeError` screen instead of silently rendering) rather than staying dormant code nothing calls. `CLASS_INDEX`/`currentAuthUid()` are forward-compatible with step 2's `data.account` shape and extend naturally once that PR merges — nothing here needs to change when it does. See `CHANGELOG.md` for the full breakdown.

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`) — both PRs are still draft.
