...
**What's actually next: an implementation session, once a real Firebase/GCP project exists to build against.** Start with step 2 (auth + Cloud Functions + `firestore.rules`) — almost every later step's Cloud Functions extend `provisionRebbi`/`isSuperadmin()`/etc. from that design. Creating the project itself is Ben's action.
