...
- **Step 4 (routing/back-button)** — PR #292, draft, CI green: all 5 PRs of its own incremental sequence, including the tier-aware route guard (PR4) built against step 2's now-merged `data.account` shape — real hash-based routing, working Back/Forward, deep links, every user-clickable tab-jump routed through `router.go()`, fail-closed `view-routeError` for an unknown/bad classId.
...
**What's left in the whole rebuild is step 8** (migrating the real beta cohort) — not started, and it waits on all of the above actually merging and deploying first.
