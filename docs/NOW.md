...
**Step 2 (auth + Cloud Functions + `firestore.rules`) is now built and in review — PR #290, draft.** Ben created the real `menchmark-backend` Firebase/GCP project (2026-08-16), which unblocked implementation. `firestore.rules` enforces the full tier-gating matrix from `Firebase_Step2_Auth_Rules_Design_Proposal.md`; three Cloud Functions (`redeemCode`, `provisionRebbi`, `viewAs`) are the only writers of `accounts`/`codes.usedBy`; `firestore.rules.test.js` is the emulator-based verification harness (23/23 assertions passing, now wired into CI as a `firestore-rules` job). **Backend only — no `app.html` changes** (sign-in UI is step 4's scope) and **nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`). One doc discrepancy surfaced while writing the test suite: `Firebase_Step2_Auth_Rules_Design_Proposal.md`'s rule spec requires sign-in to read `codes` but its harness prose says "unauthenticated" — implemented per the rule spec (matches the actual sign-in flow, where Google Sign-In always precedes any code screen); flagged in the PR rather than silently edited.

**Almost every later step's Cloud Functions extend `provisionRebbi`/`isSuperadmin()`/etc. from step 2's design** — so step 2 landing for real (not just designed) is what unblocks step 3 (the converter tool) next.
