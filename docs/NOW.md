...
**The sign-in UI gap flagged in both of the above is now closed on design — `docs/Firebase_SignIn_UI_Design_Proposal.md`, APPROVED 2026-08-16 by Ben, build in progress.** Every sign-in starts SDK-free (plain Google OAuth redirect + Firebase Auth REST API, same pattern the Drive spike already proved) — the vendored SDK loads only once an account turns out to be tier-1. Three non-wall entry points (a skippable `setup.html` step, a Settings → Account card, a one-time nudge banner), the full screen sequence matching step 2's already-locked reads/writes, and where step 3b's same-device-adoption offer surfaces. This unblocks: step 3's self-serve restore mount (backend done, waiting on this), step 4's tier-guard PR (deliberately left unbuilt — needs a real signed-in session to test), and step 3b itself.

**Nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`) — PR #290, #292 and the sign-in UI work are all still draft/in-progress.
