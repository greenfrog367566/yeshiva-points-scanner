...
**Slice 1 shipped (#248): attendance cells are editable. Slice 2 shipped (PR #285, commit `75096f2`): homework cells are editable too**, via the planned optional `dateKey` parameter on `setHw()` — same click-to-edit pattern as Attendance, Checked/Not-checked only (the Homework tab itself is still boolean), blank cells editable, screen-only. Two slices left, and it is **one PR per tab**, because the two remaining legacy setters don't have a simple "set the value for day X" shape the way Attendance/Homework do: `setPass()` keeps no per-use history, and `trackerLogAdd()` writes individual timestamped `+1` log entries rather than one record per day. **Passes may not be correctable at all** without first giving that store real history, which is a bigger question than a column. **Tracker is the same kind of question** — an edit would mean "add/remove entries until the day's count matches N," not a mechanical dateKey port. Both are PROPOSE-FIRST design questions, not EXECUTE-FREELY mechanical work.
