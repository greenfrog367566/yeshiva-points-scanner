...
**The Firebase/Firestore rebuild's full design phase is DONE (2026-08-14).** All 8 build-order steps (0 through 7) now have an approved, locked design — `docs/Firebase_Rebuild_Scope.md`'s "Open questions for step 1" list is fully resolved, and each subsequent step has its own design doc: `Firebase_DataModel_Design_Proposal.md` (1), `Firebase_Step2_Auth_Rules_Design_Proposal.md` (2), `Firebase_Step3_Converter_Tool_Design_Proposal.md` (3), `Firebase_Step3b_Existing_Cohort_Upload_Design_Proposal.md` (3b — the sign-in gate), `Firebase_Step4_Routing_Design_Proposal.md` (4), `Firebase_Step5_Superadmin_Tools_Design_Proposal.md` (5), `Firebase_Step6_Admin_Gradebook_View_Design_Proposal.md` (6), `Firebase_Step7_Fragile_Storage_Warning_Design_Proposal.md` (7). **Nothing has been built yet** — every doc ends with its own "still to build" list. Step 8 (migrate the real beta testers) is the only step left, and it's pure execution against these 7 docs.

**Steps 2 and 3 are now built and in review — PR #290, draft.** Ben created the real `menchmark-backend` Firebase/GCP project (2026-08-16), which unblocked implementation, then said "go" to keep moving into step 3 the same session.

**Step 2** (auth + Cloud Functions + `firestore.rules`): `firestore.rules` enforces the full tier-gating matrix from `Firebase_Step2_Auth_Rules_Design_Proposal.md`; three Cloud Functions (`redeemCode`, `provisionRebbi`, `viewAs`) are the only writers of `accounts`/`codes.usedBy`; `firestore.rules.test.js` is the emulator-based verification harness (31/31 assertions passing after step 3's additions). One doc discrepancy surfaced while writing the test suite: `Firebase_Step2_Auth_Rules_Design_Proposal.md`'s rule spec requires sign-in to read `codes` but its harness prose says "unauthenticated" — implemented per the rule spec (matches the actual sign-in flow, where Google Sign-In always precedes any code screen); flagged in the PR rather than silently edited.

**Step 3** (the converter tool): `provisionRebbi` now dispatches on `mode` — `admin-invite` (unchanged), `roster`, and `backup` (admin-driven or self-serve via `self:true`) — writing classes/students/trackedItems/trackedEntries/activities/log in chunked batches and storing a `classes/{id}/importReceipts/{runId}` (count parity + spot check + a `nameSplitFlags` count), proven against the real exported function on the Firestore+Auth emulators (`functions/provisionRebbi.test.js`, 8/8 passing). The Firebase SDK is vendored for real now (`vendor/firebase/*-compat.js`), and **CLAUDE.md rule 3 was amended a second time (Ben, 2026-08-16, via chat)** to let it also load in `tools/admin-convert.html` (built this pass) and step 5's `admin.html` (not yet). `tools/converter-core.js` + `tools/admin-convert.html` give a superadmin roster-only, backup-upload-with-preview, and sequential bulk-invite UI. `app.html` gained one small addition, `window.__exportNormalized()`, so a backup is always normalized through the app's own `migrateData()`/`load2fix()` rather than a second implementation. **Deliberately not built:** the self-serve restore mount inside `app.html`'s own Settings — `app.html` has no sign-in/tier detection of any kind yet, since that's step 4's unbuilt job; the backend half is ready and waiting. **Also deliberately out of scope, matching the design doc's own boundary:** `prizeLedger` (raffle/auction/store), seating, and Settings aren't wired into the converter yet — covered so far: classes, students, trackedItems, trackedEntries, activities, log, scores.

**Both CI-green, both still backend/tool-only — nothing deployed to the live project yet** (`firebase deploy` needs Ben's `firebase login`). **Almost every later step's Cloud Functions extend `provisionRebbi`/`isSuperadmin()`/etc.** — so step 3 landing for real is what unblocks step 3b (existing-cohort upload) and step 4 (routing/sign-in UI) next.
