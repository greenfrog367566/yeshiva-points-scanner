...
5. **Superadmin tools — ✅ DESIGNED, approved 2026-08-14 by Ben.** View-as, the activity overview, the email export. Full design in `docs/Firebase_Step5_Superadmin_Tools_Design_Proposal.md`: a separate `admin.html` (not a route inside `app.html`, so the rebbi-facing majority never ships this privileged surface); an explicit "Load activity" button reading an `activitySummary` doc populated by Cloud Function triggers on sign-in and any owned-data write; view-as opens in a new browser tab, never same-tab or an iframe; a single server-side Cloud Function returning a flat CSV for the email export, zero sending infrastructure; and one consistent audit trail covering every privileged read/mint/export, with the client-side role check explicitly documented as defense-in-depth only. **Still to build:** `admin.html`, its Cloud Functions, and the audit-log wiring — this locks the design, not the implementation.
6. **Admin's gradebook view — ✅ DESIGNED, approved 2026-08-14 by Ben.** Class Book default, Teacher's Book where shared. Full design in `docs/Firebase_Step6_Admin_Gradebook_View_Design_Proposal.md`: an item-level `trackedItems.book: 'class'|'teacher'` field (attendance/homework/recognition default `'class'`, everything else defaults `'teacher'`), `sharedWithAdmin` repurposed as a whole-book override alongside the per-item share, an updated `itemVisible()` security rule composing with step 2's `canRead()`/`isAdminOf()`, and an admin UI with no hidden-item counter and a non-dismissible "shared by" badge. This closes step 2's flagged open follow-on. **Still to build:** the `book` field itself, the UI, and the sharing confirms — this locks the design, not the implementation.
