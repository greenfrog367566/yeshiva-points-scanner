...
**2b. The Gradebook UI** *(SHIPPED — PR #115)* — the one full interface: auto-populated matrix, toggleable columns, search + date filter, weekly grid view, by-student drill-down, print. **Hidden from the nav since #185** until 2d gives it something to read.
...
- **#185 is unblocked. Un-hiding it is a separate PR by design** — the data change and the reveal are verified separately, and the receipt is meant to be read against the real tabs first.
