...
- **#185 shipped as its own PR**, deliberately separate from the data change so the two were verified independently. Its one visible rough edge: the Homework column starts at the ship date (decision 2 of the proposal) while the Homework tab still shows older marks — a one-line backfill addition if that reads as broken to a rebbi.
