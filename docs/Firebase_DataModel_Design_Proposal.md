...
**Guard against a cleared shadow before ever pushing.** The shadow alone is not a safe trigger: if a device's storage is cleared (Chromebook admin wipe, guest-mode reset — the exact scenario the Sheets-backup fallback already exists for), `lastKnownSchoolId` and `data.students` vanish together, while the account doc's `schoolId` stays set server-side. Read at face value, the edge condition above would then fire the converter push again — but this time with empty/default local data, overwriting the real class in Firestore with junk. So the promotion trigger is never "shadow is null" alone: **before running the converter push, check whether `classes/{ownerId}_...` already exists in Firestore.** If it does, this is a re-install on an already-migrated account, not a first migration — resync the local shadow from the remote value and skip the push entirely. Only an account with `schoolId` set and *no* existing `classes` doc for that `ownerId` is a genuine first-time promotion.
